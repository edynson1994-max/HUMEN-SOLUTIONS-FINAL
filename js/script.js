...
mostrarResultado(precio, detalle) {
    ...
    const descuentoBox = document.getElementById("descuentoPagoRapido");
    const descuentoPrecioFinal = document.getElementById("descuentoPrecioFinal");
    const descuentoPrecioOriginal = document.getElementById("descuentoPrecioOriginal");

    // Descuento por pago inmediato desde la cotización rápida
    const precioConDescuento = Math.round(precio * (1 - DESCUENTO_PAGO_RAPIDO) * 100) / 100;
    ...
    if (btnPagarAhora) {
        ...
        if (descuentoBox) {
            ...
            descuentoBox.style.display = necesitaRevision ? "none" : "flex";

            if (descuentoPrecioFinal) {
                descuentoPrecioFinal.textContent = "S/ " + precioConDescuento.toLocaleString("es-PE", {
                    minimumFractionDigits: 2,
                    maximumFractionDigits: 2
                });
            }

            if (descuentoPrecioOriginal) {
                descuentoPrecioOriginal.textContent = "S/ " + precio.toLocaleString("es-PE", {
                    minimumFractionDigits: 2,
                    maximumFractionDigits: 2
                });
            }
...
// Descuento aplicado cuando el cliente paga directo desde la cotización
// rápida (en vez de pasar por "Solicitar Cotización"). Cambia solo este
// número (ej. 0.15 para 15%) si quieres ajustar el porcentaje.
const DESCUENTO_PAGO_RAPIDO = 0.10;
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function abrir() {
        ...
        // Se calcula siempre a partir del precio actual, en vez de depender
        // de un valor guardado por separado (Cotizador.precioConDescuento),
        // para que nunca pueda quedar desincronizado con lo que se cobra.
        const precioFinal = Math.round(precio * (1 - DESCUENTO_PAGO_RAPIDO) * 100) / 100;
        Cotizador.precioConDescuento = precioFinal;

        resumen.innerHTML = `
            <strong>Servicio:</strong> ${nombreServicio(Cotizador.servicioActual)}<br>
            <strong>Código:</strong> ${Cotizador.codigoActual}<br>
            <strong>Total a pagar (10% dcto. por pago inmediato):</strong> S/ ${precioFinal.toLocaleString("es-PE", {minimumFractionDigits:2, maximumFractionDigits:2})}
        `;
    ...
    form.addEventListener("submit", async (e) => {
        ...
        const datos = {
            ...
            descuentoAplicado: DESCUENTO_PAGO_RAPIDO,
